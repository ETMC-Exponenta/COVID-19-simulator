classdef VirusPropagationDev < handle
    ...
    methods
        function obj = VirusPropagationDev(extender)
            % Init
            if nargin < 1
                obj.ext = VirusPropagationExtender;
            else
                if ischar(extender) || isStringScalar(extender)
                    obj.ext = VirusPropagationExtender(extender);
                else
                    obj.ext = extender;
                end
            end
        ...
        function check(obj)
            ...
            upd = VirusPropagationUpdater(obj.ext);
            upd.fetch();
